...
app.loadDataOnPage = () => {
    ...
    const primaryClass = typeof(bodyClasses[0]) === 'string' ?
        bodyClasses[0] : false;
...
app.loadDataOnPage = () => {
    ...
    const phone = typeof(app.config.sessionToken.phone) === 'string' ?
        app.config.sessionToken.phone : false;
    if (phone) {
        // Fetch the user data
        const queryStringObject = {
            'phone': phone
        ...
        app.client.request(undefined, 'api/users', 'GET', queryStringObject, undefined, (statusCode, responsePayload) => {
            if (statusCode === 200) {
                ...
                document.querySelector('#accountEdit1 input[name="firstName"]').value = responsePayload.firstName;
                document.querySelector('#accountEdit1 input[name="lastName"]').value = responsePayload.lastName;
                document.querySelector('#accountEdit1 input[name="displayPhone"]').value = responsePayload.phone;

                // Put the hidden phone field into both forms
                const hiddenPhoneInputs = [];
                hiddenPhoneInputs.push(document.querySelector('#accountEdit1 input[name="phone"]'));
                hiddenPhoneInputs.push(document.querySelector('#accountEdit2 input[name="phone"]'));
